...
settings.output_list = [
    ...
    tensor(identity(settings.nq0.value), create(settings.nq1.value) + destroy(settings.nq1.value)),
    tensor(identity(settings.nq0.value), -1j*(-create(settings.nq1.value) + destroy(settings.nq1.value))),
]
...
def create_hamiltonian(s):
    ...
    s.w0_coeff = w0_coeff
    s.w1_coeff = w1_coeff
    s.psi0 = tensor((basis(s.nq0.value, 1)*s.q0_t0.value + basis(s.nq0.value, 0)*(1-s.q0_t0.value)).unit(), (basis(s.nq1.value, 1)*s.q1_t0.value  + basis(s.nq0.value, 0)*(1-s.q1_t0.value)).unit())


def task(H, H1_0, H1_1, w0_coeff, w1_coeff, psi0, tlist, c_ops, output_list, wd0, wd1, w0, w1):
    """
    ...
    output[f"phase"] = np.unwrap(np.angle((np.real(output[f"q2"]) + 1j*np.real(output[f"q3"]))*np.exp(1j*wd0*tlist)))
    return output
